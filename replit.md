...
- **Sunday School Teachings** (`/sunday-school`) — Standalone main-menu page with 24 ready-to-use lessons (8 Children, 8 Youth, 8 Adult) covering core themes (Creation, Obedience, Courage, Prayer, Faith, Love, Identity, Holiness, Friendship, Stewardship, Mission, etc.). Each lesson has a Key Verse, 3 numbered Teaching Points, Discussion Questions, an Activity idea, and a Closing Prayer — every section has its own Send-to-screen button. Master/detail layout with age-group filter, theme pills, and search. Data lives in `lib/sunday-school.ts` (pure local, offline-friendly).
- **How To** (`/how-to`) — In-app user guide. Searchable accordion covering every feature (Bible, Songs, Custom Text, Themes, Media & Broadcast, Schedule, Sermon Notes, Daily Inspiration, Sunday School, Settings, Live Preview Sidebar) with quick-jump tiles at the top, numbered steps, pro tips, and an "Open page" link per section. Open-state persists via `wf-howto-open`.
