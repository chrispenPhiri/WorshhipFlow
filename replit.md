...
*   **Install & Local Storage (Settings card):** When installed as a PWA, the app actively makes use of the host PC's local storage. Three concerns, three modules under `src/lib/backup/`:
    - `persistent.ts` wraps `navigator.storage.persist()` and `persisted()`. `src/main.tsx` fire-and-forgets a silent persist request whenever `isInstalled()` is true on boot, so the installed app gets the "won't be auto-evicted" guarantee without the user doing anything. The Settings card also exposes a manual "Protect storage" button and shows the current state (`persisted` / `transient` / `unsupported`) as a colored pill.
    - `snapshot.ts` builds a single-file backup of the entire local library (songs, notes, schedules, singletons minus the backup-folder handle, and local user accounts including their PBKDF2 hashes — never plaintext passwords). `downloadSnapshot()` prefers `showSaveFilePicker` (real Save-As dialog) and gracefully falls back to a Blob+anchor download for Safari/Firefox. `pickAndRestoreSnapshot()` validates the file (`app === "phiri-worshipflow-backup"`, version ≤ current) and atomically replaces every store via the new `replaceAll()` helper in `local-api/db.ts` (single readwrite transaction per store, so a partial failure can't leave the library half-restored). The backup-folder handle in `singletons` is preserved across a restore so the user doesn't have to reconnect.
    - `folder.ts` is the File System Access integration. `connectBackupFolder()` calls `showDirectoryPicker({mode:"readwrite"})` and persists the granted `FileSystemDirectoryHandle` (structured-cloneable) in IDB `singletons` under `_backup:folder` along with its display name. `saveBackupToFolder()` writes a rolling `worshipflow-backup.json` at the folder root *and* a timestamped copy under `history/`, then prunes oldest history entries beyond 10. Permissions are re-queried on every action (`queryPermission` → `requestPermission` if dropped); when access is lost across reloads the UI swaps the "Save backup now" button for a "Reconnect folder" button (which works because clicking is a user gesture). `restoreFromFolder()` reads the rolling file back. The whole section is hidden behind `isFolderApiSupported()` (Chrome/Edge only) with copy directing Firefox/Safari users to the export-to-file flow.
    The card itself (`install-app-card.tsx`) is reorganised into four status pills (Installed / Online / Storage usage / Storage protection), then sectioned into Protect → Export-import → Folder backup, each with `data-testid`s on every interactive element. A shared `withBusy()` helper guards each async action and routes errors to toast (cancelled-by-user dialogs are swallowed silently). A confirm AlertDialog gates both restore paths so the user can't accidentally overwrite their library.

*   **Local Accounts (Auth Gate):** The whole control app (everything except `/broadcast`) is gated behind a local-only username/password sign-in. Accounts live entirely in this device's IndexedDB — credentials are never transmitted. Implementation: `src/lib/auth/crypto.ts` hashes passwords with PBKDF2-SHA256 (200,000 iterations, 16-byte random salt, 32-byte derived key, base64 storage, constant-time verify); `src/lib/auth/store.ts` persists `UserRecord`s in a new `users` IDB store (DB bumped to v2, indexed on `usernameLower` for case-insensitive uniqueness) and writes the active session to `localStorage["wf-auth-session"]` plus a `BroadcastChannel("wf-auth-session")` for cross-tab sync; `src/lib/auth/context.tsx` exposes `<AuthProvider>` and `useAuth()` (with hydration guard so the login screen doesn't flash for already-signed-in users). `src/pages/auth.tsx` is a single tabbed login/signup card that defaults to "Create account" on a fresh device and "Sign in" once any user exists, with inline validation (username 3–32 chars `[a-zA-Z0-9_.\-]`, password ≥ 6, confirm-match) and a "Local accounts only" disclaimer. The sidebar's `<UserMenu>` shows the current display name + `@username` and a logout button (compact icon-only when the desktop sidebar is collapsed). `/broadcast` is intentionally exempt from the gate so the projection window opened by an already-signed-in operator keeps working.
...
*   **Pages:**
    ...
    *   **Songs:** Library management with search, categories, and full CRUD for songs. Lyrics are split into slides, with color-coded section chips (verse, chorus, bridge, pre-chorus). Supports **bulk import** via the `<SongImportDialog>` (next to "Add Song"): users can download a JSON or CSV template prefilled with example songs (Amazing Grace, 10,000 Reasons), edit it, and upload it back. The dialog parses both formats (RFC4180-ish CSV with quoted fields, embedded newlines, doubled-quote escapes, and BOM stripping; JSON array or single object), validates required fields (`title`, `author`, `lyrics`), normalises the category against the allowed enum (falling back to "other"), and bulk-creates each song through the existing `useCreateSong` hook into IndexedDB. A monotonic session-id ref guards against stale async file-reads and stale post-import state if the user closes/reopens the dialog mid-flight; the dialog cannot be dismissed (escape, outside click, X) while an import is in progress.
    ...
    *   **Bible Games:** Ten offline games — Bible Trivia, Books of the Bible (ordering), Who Said It?, Bible Charades, Verse Scramble (KJV), Bible Emoji Quiz, Bible Hangman, True or False (Easy/Medium/Hard/Mixed), Bible Spell-It (clue + letter pool), and Two Truths and a Lie. Most games include "Show on screen" and "Reveal answer on screen" buttons that project the puzzle/question/answer onto the broadcast window. Projection uses a structured `contentType: "game"` payload (added to the OpenAPI enum) — `lib/game-stage-payload.ts` defines a discriminated union (`GameStagePayload`) with one variant per game, and `lib/game-broadcast.ts#presentGameView()` JSON-encodes it into screen state. Both the broadcast (`pages/broadcast.tsx`) and the operator's live preview (`components/live-preview.tsx`) detect this content type and render `<GameStageView>` (`components/game-stage-view.tsx`) — an em-based scalable card that draws option blocks (A/B/C/D pills with green/red highlight on reveal), hangman lives + masked word, charades prompt, emoji quiz tiles, scrambled verse word-tiles, spell-it letter slots, etc., at the operator's chosen font size × textScale. Decode is shape-validated per `kind` so any malformed payload silently falls back to the legacy plain-text render.
